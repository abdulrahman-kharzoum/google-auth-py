...
No MongoDB, no n8n, no file processing - Google Auth only
...
async def google_callback(code: str, state: str):
    ...
    if state not in oauth_states:
        return HTMLResponse(
            content="<script>window.opener.postMessage({ authStatus: 'error', error: 'Invalid state' }, '*'); window.close();</script>",
            status_code=400
        )
    ...
    try:
        ...
        async with httpx.AsyncClient() as http_client:
            ...
            # Post message to opener window and close pop-up
            return HTMLResponse(
                content=f"<script>window.opener.postMessage({{ authStatus: 'success', userId: '{userinfo['id']}' }}, '*'); window.close();</script>",
                status_code=200
            )
            
    except Exception as e:
        # Post error message to opener window and close pop-up
        error_message = urllib.parse.quote(str(e))
        return HTMLResponse(
            content=f"<script>window.opener.postMessage({{ authStatus: 'error', error: '{error_message}' }}, '*'); window.close();</script>",
            status_code=400
        )
